import {
  ...
  import { generateOutputComplexTypeAsArray, generateOutputComplexTypesAsObject } from './types'
  ...
  function generateErrorTypeExport(error: CustomErrorDeclaration, includeArgTypes: boolean) {
    const components = error.inputs.map((input, i) => ({ name: input.name ?? `arg${i.toString()}`, type: input.type }))
  ...
  export function generateGetError(error: CustomErrorDeclaration, useSignature: boolean): string {
    return `getError(nameOrSignature: '${
      useSignature ? generateErrorSignature(error) : error.name
    }'): ErrorFragment;`
  }
  
  function generateErrorIdentifier(error: CustomErrorDeclaration, { includeArgTypes }: { includeArgTypes?: boolean } = {}) {
    if (includeArgTypes) {
